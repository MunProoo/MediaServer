...
func (obj *StorageST) StreamChannelMake(val ChannelST) ChannelST {
	...
	if err := mergo.Merge(&channel, val); err != nil {
		...
		channel = val
		log.Printf("[ERROR] [storage] [StreamChannelMake] [mergo.Merge] %s", err.Error())
	}
	//make client's
...
func (obj *StorageST) StreamChannelRunAll() {
	...
	for k, v := range obj.Streams {
		for ks, vs := range v.Channels {
			// 녹화 상태가 ON이거나 OnDemand가 아닌 경우 스트림 시작
			if !vs.OnDemand || vs.OnRecording {
				vs.runLock = true
				go StreamServerRunStreamDo(k, ks, v.Name)
				v.Channels[ks] = vs
				obj.Streams[k] = v
			}
		}
	...
	if streamTmp, ok := obj.Streams[streamID]; ok {
		if channelTmp, ok := streamTmp.Channels[channelID]; ok {
			if !channelTmp.runLock {
				...
				go StreamServerRunStreamDo(streamID, channelID, streamTmp.Name)
			}
		}
	}
}
...
func (obj *StorageST) StreamChannelAdd(uuid string, channelID string, val ChannelST) error {
	...
	streamInfo := obj.Streams[uuid]

	if _, ok := streamInfo.Channels[channelID]; ok {
		return ErrorStreamChannelAlreadyExists
	}
	val = obj.StreamChannelMake(val)
	streamInfo.Channels[channelID] = val
	if !val.OnDemand {
		val.runLock = true
		go StreamServerRunStreamDo(uuid, channelID, streamInfo.Name)
	}
	err := obj.SaveConfig()
	if err != nil {
		return err
...
func (obj *StorageST) StreamChannelEdit(uuid string, channelID string, val ChannelST) error {
	...
	if tmp, ok := obj.Streams[uuid]; ok {
		if currentChannel, ok := tmp.Channels[channelID]; ok {
			...
			if !val.OnDemand {
				val.runLock = true
				go StreamServerRunStreamDo(uuid, channelID, tmp.Name)
			}
			err := obj.SaveConfig()
			if err != nil {
				return err
...
// StreamChannelRecordingStatus update stream channel recording status
func (obj *StorageST) StreamChannelRecordingEnabled(streamID string, channelID string, status bool) {
	obj.mutex.Lock()
	defer obj.mutex.Unlock()
	if tmp, ok := obj.Streams[streamID]; ok {
		if channelTmp, ok := tmp.Channels[channelID]; ok {
			channelTmp.OnRecording = status
			tmp.Channels[channelID] = channelTmp
			obj.Streams[streamID] = tmp
