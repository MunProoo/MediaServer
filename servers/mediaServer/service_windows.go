...
func (m *msService) Execute(args []string, req <-chan svc.ChangeRequest, status chan<- svc.Status) (svcSpecificExitCode bool, exitCode uint32) {
	...
	for {
		...
		case c := <-req:
			...
			case svc.Stop, svc.Shutdown:
				...
				// 정리 작업이 완료될 때까지 충분한 시간 대기
				log.Printf("[INFO] [main] Waiting for cleanup to complete...")
