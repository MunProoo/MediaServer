  ...
  Swal.fire({
    ...
    preConfirm: () => {
      var uuid = randomUuid(),
        ...
        url = $('#stream-url').val(),
        ondemand = $('#stream-ondemand').is(':checked');
      if (!validURL(url)) {
        Swal.fire({
          icon: 'error',
          title: 'Oops...',
...
function goRequest(method, uuid, data) {
  ...
  switch (method) {
    ...
    case 'recording-toggle':
      path = '/stream/' + uuid + '/channel/0/recording/toggle';
      type = 'POST';
      break;
    case 'recording-start':
      path = '/stream/' + uuid + '/channel/0/recording/start';
      type = 'POST';
      break;
    case 'recording-stop':
      path = '/stream/' + uuid + '/channel/0/recording/stop';
      type = 'POST';
      break;
    default:
      path = '';
...
function goRequestHandle(method, response, uuid) {
  switch (method) {
      ...
      if (response.status == 1) {
        ...
        if (Object.keys(streams).length > 0) {
          ...
          $.each(streams, function(uuid, param) {
            if ($('#' + uuid).length == 0) {
              // channels에서 첫 번째 채널의 녹화 상태를 가져옴 (기본 채널 0)
              var recordingStatus = 1; // 기본값: 녹화 중지
              if (param.channels && param.channels['0'] && typeof param.channels['0'].recording_status !== 'undefined') {
                recordingStatus = param.channels['0'].recording_status;
              }
              $('.streams').append(streamHtmlTemplate(uuid, param.name, recordingStatus));
            } else {
              // 기존 스트림의 녹화 상태 업데이트
              updateStreamRecordingStatus(uuid, param);
            }
          })
        }
      }
    ...
    case 'recording-stop':
      if (response.status == 1) {
        renewStreamlist();
        var actionText = method === 'recording-start' ? '시작' : (method === 'recording-stop' ? '중지' : '토글');
        Swal.fire(
          '성공!',
          '녹화가 ' + actionText + ' 되었습니다.',
          'success'
        );
      } else {
        Swal.fire({
...
function streamHtmlTemplate(uuid, name, recordingStatus) {
  recordingStatus = recordingStatus || 0;
  var recordingIcon = '';
  var recordingText = '';
  
  switch(recordingStatus) {
    case 0: // RecordingOn
      recordingIcon = '<i class="fas fa-circle text-danger" title="녹화 중"></i>';
      recordingText = '녹화 중';
      break;
    case 1: // RecordingOff  
      recordingIcon = '<i class="fas fa-circle text-secondary" title="녹화 중지"></i>';
      recordingText = '녹화 중지';
      break;
    case 2: // RecordingErr
      recordingIcon = '<i class="fas fa-exclamation-triangle text-warning" title="녹화 오류"></i>';
      recordingText = '녹화 오류';
      break;
  }
  ...
  return '<div class="item" id="' + uuid + '">' +
    ...
    '</div>' +
    '<div class="text">' +
    '<h5>' + name + ' <span class="recording-status">' + recordingIcon + '</span></h5>' +
    '<p>상태: <span class="recording-text">' + recordingText + '</span></p>' +
    ...
    '<a class="dropdown-item" onclick="toggleRecording(\'' + uuid + '\')" href="#" id="recording-toggle-' + uuid + '">녹화 제어</a>' +
...
// 스트림 녹화 상태 업데이트
function updateStreamRecordingStatus(uuid, streamData) {
  var recordingStatus = 1; // 기본값: 녹화 중지
  if (streamData.channels && streamData.channels['0'] && typeof streamData.channels['0'].recording_status !== 'undefined') {
    recordingStatus = streamData.channels['0'].recording_status;
  }
  ...
  $('#' + uuid + ' .recording-status').html(recordingIcon);
  $('#' + uuid + ' .recording-text').text(recordingText);
}

// 녹화 시작/중지 토글
function toggleRecording(uuid) {
  goRequest('recording-toggle', uuid);
}

// 녹화 시작
function startRecording(uuid) {
  goRequest('recording-start', uuid);
}

// 녹화 중지
function stopRecording(uuid) {
  goRequest('recording-stop', uuid);
}
